...
deltarsl_mean_u = 0.0066 * 150.* 0.
deltarsl_mean_v = 0.0066 * 150.* 0.
deltarsl_variance_u = 0.0066 * 150. * 9.
deltarsl_variance_v = 0.0066 * 150. * 9.
deltarsl_covariance = 0.0066 * 150.* 10.
deltarsl_skewness_u = 0.0066 * 150.* 5.
deltarsl_skewness_v = 0.0066 * 150.* 0.
deltaresl_kurtosis_u = 0.0066 * 150.* 3.
deltaresl_kurtosis_v = 0.0066 * 150.* 5.
# mean = 6.83
    ...
    for var_name in var_name_list:
        ...
        for i,name in enumerate(namelist):
            ...
            if var_name == 'u':
                ...
                if i==1:
                    ax.vlines(deltarsl_mean_u, -0.5, 1.25, colors='tab:red', 
                            linestyles='dashed', 
                ...
                ax.errorbar(wt_z_plot, wt_var_plot, yerr = wt_err[name]['vmean'],
                            ...
                            fmt=marker_list[i], color=c_list[i])
                if i==1:
                    ax.vlines(deltarsl_mean_v, -0.05, 0.25, colors='tab:red', 
                            linestyles='dashed', 
                            label=r'$\delta_{rsl}(h_r)$')
                ax.set_ylabel(r'$\overline{v}$ $u_{ref}^{-1}$ (-)')
        ax.grid(False,'both', 'both')
        ax.set_xlim(0.08, 80.)
        ax.legend(bbox_to_anchor = (0.,1.1), loc = 'lower left', 
                    borderaxespad = 0.,  
                    numpoints = 1, ncol = 5)
...
if compute_front_highermoments:
    ...
    for var_name in var_name_list:
        ...
        for i,name in enumerate(namelist):
            ...
            wt_z_plot = np.asarray(wt_z)-0.115*scale
            if var_name == 'u':
                ax.errorbar(wt_z_plot, wt_skew_weight, yerr = err,
                            label=label_list[i], 
                            fmt=marker_list[i], color=c_list[i])
                if i==1:
                ...
                ax.errorbar(wt_z_plot, wt_skew_weight, yerr = err,
                            ...
                            fmt=marker_list[i], color=c_list[i])
                if i==1:
                    ax.vlines(deltarsl_skewness_v, -0.8, 1., colors='tab:red', 
                            linestyles='dashed', 
                            label=r'$\approx \delta_{rsl}$')
        ...
        for i,name in enumerate(namelist):
            ...
            wt_z_plot = np.asarray(wt_z)-0.115*scale
            if var_name == 'u':
                ax.errorbar(wt_z_plot, wt_kurt_weight, yerr = err,
                            label=label_list[i], 
                            fmt=marker_list[i], color=c_list[i])
                if i==1:
                    ax.vlines(deltaresl_kurtosis_u, 2, 7, colors='tab:red', 
                            linestyles='dashed', 
                            label=r'$\approx \delta_{rsl}$')
                ...
                ax.errorbar(wt_z_plot, wt_kurt_weight, yerr = err,
                            ...
                            fmt=marker_list[i], color=c_list[i])
                if i==1:
                    ax.vlines(deltaresl_kurtosis_v, 2.5, 7, colors='tab:red', 
                            linestyles='dashed', 
                            label=r'$\approx \delta_{rsl}$')
    ...
    for var_name in var_name_list:
        ...
        for i,name in enumerate(namelist):
            ...
            if var_name == 'u':
                ...
                if i==1:                            
                    # vertical line
                    ax.vlines(deltarsl_variance_u, 0., 0.3, colors='tab:red', 
                            linestyles='dashed', 
                            label=r'$\approx \delta_{rsl}$')
                ...
                if i==1:
                    # vertical line
                    ax.vlines(deltarsl_variance_v, 0., 0.08, colors='tab:red', 
                            linestyles='dashed', 
                            label=r'$\approx \delta_{rsl}$')
...
if compute_front_covar:
    ...
    # vertical line
    ax.vlines(deltarsl_covariance, -0.06, 0., colors='tab:red', 
                linestyles='dashed', 
                label=r'$\approx \delta_{rsl}$')                
    ...
    for i,mask in enumerate(mask_name_list):
        ...
        if plot_QA_PALM:
            ...
            fig.savefig('../palm_results/{}/run_{}/quadrant_analysis/jpdf/{}_QA_jpdf_mask_{}.{}'.format(papy.globals.run_name,
                        papy.globals.run_number[-3:],
                        'front', mask + '_' + str(wall_dist[0]), plotformat), bbox_inches='tight', dpi=500)
            print('     SAVED TO: ' 
                        + '../palm_results/{}/run_{}/quadrant_analysis/jpdf/{}_QA_jpdf_mask_{}.{}'.format(papy.globals.run_name,
                        papy.globals.run_number[-3:],
                        'front', mask + '_' + str(wall_dist[0]), plotformat))
            plt.close()
            ...
            for j,file in enumerate(files):
                ...
                if plot_WT_QA:
                    ...
                    fig.savefig('../palm_results/{}/run_{}/quadrant_analysis/jpdf/{}_QA_jpdf_WT_{}.{}'.format(papy.globals.run_name,
                                papy.globals.run_number[-3:],
                                'front', file + '_' + str(wt_wall_dist[0])[:5], plotformat), bbox_inches='tight', dpi=500)
                    print('     SAVED TO: ' 
                                + '../palm_results/{}/run_{}/quadrant_analysis/jpdf/{}_QA_jpdf_WT_{}.{}'.format(papy.globals.run_name,
                                papy.globals.run_number[-3:],
                                'front', file + '_' + str(wt_wall_dist[0])[:5], plotformat))
                    plt.close()
